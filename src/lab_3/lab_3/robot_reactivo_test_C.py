...
class Robot_Reactivo_test(Node):

    def __init__(self):
        super().__init__('robot_reactivo_test')
        ...
        self.mcl_sub = self.create_subscription(Float64, 'MCL', self.señal_run, 10)
        self.react_publisher = self.create_publisher(Bool, "reactive", 10)
        ...
        #file_path = 'ros2_ws/src/my_first_packege/my_first_packege/mapa.pgm'
        self.declare_parameter( 'file_path', 'ros2_ws/src/my_first_packege/my_first_packege/mapa.pgm' )
    ...
    def deteccion_obstaculo(self, scan):
        ...
        self.end -= 1
        # print(self.end)
        if self.end <= self.tiempo_exploracion:
            self.espera = True
            msg = Bool()
            msg.data = bool(True)
            self.react_publisher.publish(msg)

    def señal_run(self, tiempo):
        self.tiempo_exploracion = 0
        self.end = tiempo.data
        self.espera = False
...
def main(args=None):
    ...
    reactive_movement_node = Robot_Reactivo_test()
